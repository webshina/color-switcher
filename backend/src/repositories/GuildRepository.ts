...
export class GuildRepository {
  static async format(
    ...
    const managementMembers =
      await GuildMemberRepository.getManagementMembersByGuildId(guildData.id);
    ...
    const guildItem: GuildItem = {
      ...
      autoGenerateManagerPost: guildData.autoGenerateManagerPost,
      coverImageUrl:
  ...
  static async generateDescription(props: {
    ...
    if (!updatedMessage && guildData.description) {
      await updateBatchProgress();
      return;
    }

    if (guildData.autoGenerateDescription) {
      const topActivityChannels = await prisma.channel.findMany({
        where: {
          guildId: guildData.id,
        },
        include: {
      ...
      const materialsForDescription = JSON.stringify(
        topActivityChannels.map((channel) => {
          return {
            name: channel.name,
      ...
      const prompt = `Create description of Discord server using following data. 
...
`;
  ...
  static async generateTags(props: { guildId: number; batchId: number }) {
    ...
    if (!updatedMessage && guildTags.length > 0) {
      await updateBatchProgress();
      return;
    }
